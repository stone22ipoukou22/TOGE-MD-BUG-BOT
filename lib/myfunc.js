...
exports.buffergif = async (image) => {
	...
	const filename = `${Math.random().toString(36)}`
			await fs.writeFileSync(`./TOGEMedia/trash/${filename}.gif`, image)
					 child_process.exec(
								`ffmpeg -i ./TOGEMedia/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./TOGEMedia/trash/${filename}.mp4`
										) 
  await sleepy(4000)
  
	var buffer5  =  await  fs.readFileSync(`./TOGEMedia/trash/${filename}.mp4`)
	Promise.all([unlink(`./TOGEMedia/video/${filename}.mp4`), unlink(`./TOGEMedia/gif/${filename}.gif`)])
	return buffer5
				   }
...
exports.GIFBufferToVideoBuffer = async (image) => {
    ...
    var buffer5 = await fs.readFileSync(`./TOGEMedia/trash/${filename}.mp4`)
    Promise.all([unlink(`./TOGEMedia/trash/${filename}.mp4`), unlink(`./TOGEMedia/trash/${filename}.gif`)])
    return buffer5
}
